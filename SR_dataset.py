...
class ImageInfo():
    def __init__(self,x_off,y_off,ms,option):
        self.x_off=x_off
        self.y_off=y_off
        self.ms=ms
        self.option=option
        ...
        self.label=0
    def to_dict(self):
        return {'x_off':self.x_off,'y_off':self.y_off,'ms':self.ms,\
                'max_pixel':self.max_pixel,'min_pixel':self.min_pixel,'label':self.label}
class GF2(Dataset):
    def __init__(self,ms_path,patch_size,transfrom=None,option='Train'):
        self.ms=gdal.Open(ms_path)
        self.patch_size=patch_size
        self.option=option
        # self.patch_num=patch_num
        self.transform=transfrom
        self.x_limit=self.ms.RasterXSize
        self.y_limit=self.ms.RasterYSize
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            return self.transform(ImageInfo(x_off,y_off,ms,self.option))